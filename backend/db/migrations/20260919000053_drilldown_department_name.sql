-- analytics_cycle_drilldown returned a bare department_id UUID and a
-- record_id with no human-readable counterpart — useless to a viewer
-- (see 20260909000019_cycle_analytics.sql for the original). Replaces
-- department_id with a joined department_name in the final projection only;
-- record_id is left in place (frontend now hides both from the rendered
-- table, record_id has no legitimate display use at all).
