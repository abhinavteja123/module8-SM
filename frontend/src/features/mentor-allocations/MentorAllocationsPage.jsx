...
import { HourglassMedium, UsersThree, ChalkboardTeacher, CaretRight } from '@phosphor-icons/react';
...
const breadcrumb = [{ label: 'Home', to: '/coordinator' }, { label: 'Mentor Allocations' }];
...
function AllocationCard({ mapping, mentors, canAllocate, onViewStudent, onAllocated, index = 0 }) {
...
export default function MentorAllocationsPage() {
  ...
  const { data, isLoading, error, dataUpdatedAt } = useQuery({ queryKey: ['mentor-allocations', selectedCycleId], queryFn: () => api(`/mentor-allocations?cycle_id=${selectedCycleId}`), enabled: !!selectedCycleId });
  ...
  return <div className="max-w-6xl space-y-6"><PageHeader eyebrow={`Mentor management · ${selectedCycle?.name ?? 'Selected cycle'}`} title="Mentor allocations" description={canAllocate ? 'Assign a faculty mentor after CRCS approval. The student is notified immediately; hierarchy, deadlines, report access, and deadline reminders then follow automatically.' : 'Review the post-approval mentor assignments available within your role and scope.'} breadcrumb={breadcrumb} lastUpdated={lastUpdated} />
    ...
    <div className="grid gap-4 sm:grid-cols-3"><StatCard label="Needs allocation" value={waiting.length} hint="Approved students waiting for a mentor" tone="amber" icon={HourglassMedium} /><StatCard label="Allocated" value={allocated.length} hint="Students ready for supervision" tone="emerald" icon={UsersThree} /><StatCard label="Mentors with capacity" value={mentors.length} hint="Available for CRCS and self-internships" tone="indigo" icon={ChalkboardTeacher} /></div>
