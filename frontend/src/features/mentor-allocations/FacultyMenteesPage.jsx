...
function AttendanceMarker({ mapping, cycleId }) {
  const queryClient = useQueryClient();
  ...
  const { data: attendance } = useQuery({ queryKey: ['attendance', mapping.student_id, cycleId], queryFn: () => api(`/attendance/${mapping.student_id}?cycle_id=${cycleId}`), enabled: !!cycleId, retry: false });
  ...
  useEffect(() => { setWeek(firstUnmarked); }, [firstUnmarked, mapping.student_id]);
  const mark = useMutation({
    mutationFn: () => api('/attendance', { method: 'PUT', body: { cycle_id: cycleId, student_id: mapping.student_id, related_entity_type: relatedEntityType, related_entity_id: mapping.id, week_number: Number(week), present } }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['attendance', mapping.student_id, cycleId] }),
  });
  ...
  return <div className="max-w-5xl space-y-6"><PageHeader breadcrumb={[{ label: 'Home' }, { label: 'Faculty' }, { label: 'My mentored students' }]} eyebrow={`Student supervision · ${selectedCycle?.name ?? 'Selected cycle'}`} title="My mentored students" description="Choose a project or internship to see every student allocated to you, their full details, and weekly attendance." />
    ...
    {filtered.length > 0 && <div className="space-y-4">{filtered.map((mapping, index) => { const student = mapping.student ?? {}; const key = `${mapping.type}:${mapping.id}`; const options = optionsFor(mapping); return <motion.div key={key} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25, delay: Math.min(index, 8) * 0.04 }}><Card className="p-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="min-w-0"><button type="button" onClick={() => setSelectedStudent({ ...student, company_name: mapping.company_name, outcome: mapping.outcome })} className="inline-flex items-center gap-1.5 font-bold text-brand-700 hover:underline"><UserCircle size={16} weight="bold" />{student.full_name ?? 'Student'}</button><p className="mt-1 text-sm text-slate-600">{student.email}</p><p className="mt-1 text-xs text-slate-500">{student.roll_number ?? 'Roll number not provided'}{student.department?.name ? ` · ${student.department.name}` : ''}</p><p className="mt-1 text-xs font-medium text-brand-700">{mapping.title}</p></div>
        <div className="flex flex-wrap items-center gap-2"><AttendanceBadge studentId={mapping.student_id} cycleId={selectedCycleId} /><Button variant="secondary" className="px-3 py-1.5 text-xs" onClick={() => setSelectedStudent({ ...student, company_name: mapping.company_name, outcome: mapping.outcome })}>Full details</Button>{options.length > 0 && <Button variant="ghost" className="inline-flex items-center gap-1 px-3 py-1.5 text-xs text-brand-700" onClick={() => setReassigningKey(reassigningKey === key ? null : key)}><ArrowsLeftRight size={13} weight="bold" />Reassign to another faculty</Button>}</div>
      </div>
