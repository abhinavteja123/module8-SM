...
export default function DirectMentorDashboard() {
  ...
  if (!selectedCycleId) return <div className="max-w-3xl"><PageHeader eyebrow="Student supervision" title="No open cycle yet" description="Your supervision dashboard appears after CRCS publishes a cycle and assigns students to you." /></div>;
  return <div className="max-w-5xl space-y-6"><PageHeader eyebrow={`Student supervision · ${selectedCycle?.name ?? 'Selected cycle'}`} title="CRCS and self-internship mentor dashboard" description="Use the same faculty workspace to manage your assigned CRCS-opportunity and self-internship students: deadlines, report feedback, and academic marks." />
