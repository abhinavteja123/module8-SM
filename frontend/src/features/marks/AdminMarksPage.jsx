...
export default function AdminMarksPage() {
  ...
  const schoolById = Object.fromEntries(schools.map((school) => [school.id, school])); const departmentById = Object.fromEntries(departments.map((department) => [department.id, department])); const assessmentByStudent = Object.fromEntries(marks.map((row) => [row.student_id, row.assessment])); const attendanceByStudent = attendanceData.attendance ?? {};
  const requirementColumns = marks[0]?.assessment?.requirements ?? [];
  const allRows = useMemo(() => users.filter((user) => user.roles?.some((role) => role.role === 'student')).map((student) => { const studentRole = student.roles.find((role) => role.role === 'student'); const department = departmentById[studentRole?.department_id]; const school = schoolById[department?.school_id]; return { student, department, school, assessment: assessmentByStudent[student.id], attendance: attendanceByStudent[student.id] ?? null, internship: internshipFor(student.id, research, selfInternships, opportunities) }; }), [users, departmentById, schoolById, assessmentByStudent, attendanceByStudent, research, selfInternships, opportunities]);
