...
export default function AdminMarksPage({ forceFacultyCoordinatorScope = false }) {
  ...
  const { data: marks = [], isLoading: marksLoading } = useQuery({ queryKey: ['all-student-marks', cycle?.id, actAsQuery], queryFn: () => api(`/marks?cycle_id=${cycle.id}${actAsQuery}`), enabled: !!cycle?.id });
