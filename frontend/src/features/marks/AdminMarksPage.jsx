...
const breadcrumb = [{ label: 'Home', to: '/coordinator' }, { label: 'Student Marks' }];
...
export default function AdminMarksPage({ forceFacultyCoordinatorScope = false }) {
  ...
  const { data: marks = [], isLoading: marksLoading, dataUpdatedAt } = useQuery({ queryKey: ['all-student-marks', cycle?.id, actAsQuery], queryFn: () => api(`/marks?cycle_id=${cycle.id}${actAsQuery}`), enabled: !!cycle?.id });
  ...
  const schoolById = Object.fromEntries(schools.map((school) => [school.id, school])); const departmentById = Object.fromEntries(departments.map((department) => [department.id, department])); const attendanceByStudent = attendanceData.attendance ?? {};
  const requirementColumns = marks[0]?.assessment?.requirements ?? [];
  const allRows = useMemo(() => marks.map((mark) => { const student = mark.student; const department = departmentById[student?.department_id]; const school = schoolById[department?.school_id ?? student?.school_id]; return { student, department, school, assessment: mark.assessment, attendance: attendanceByStudent[student?.id] ?? null, internship: internshipFor(student?.id, research, selfInternships, opportunities) }; }).filter((row) => ['crcs_approved', 'active'].includes(row.internship.status)), [marks, departmentById, schoolById, attendanceByStudent, research, selfInternships, opportunities]);
  const rows = useMemo(() => allRows.filter((row) => { const rowStatus = row.internship.status ?? 'none'; const matchesMarks = requirementColumns.every((requirement) => { const filter = markFilters[requirement.id]; if (!filter) return true; const score = row.assessment?.requirements?.find((item) => item.id === requirement.id)?.score; return filter === 'entered' ? isMarkEntered(score) : !isMarkEntered(score); }); return (!schoolId || row.school?.id === schoolId) && (!departmentId || row.department?.id === departmentId) && (!internship || row.internship.type === internship) && (!internshipStatus || rowStatus === internshipStatus) && attendanceMatches(row.attendance, attendanceFilter) && matchesMarks && (!search.trim() || `${row.student.full_name} ${row.student.email}`.toLowerCase().includes(search.toLowerCase())); }), [allRows, requirementColumns, schoolId, departmentId, internship, internshipStatus, attendanceFilter, markFilters, search]);
  ...
  return <div><PageHeader eyebrow="Academic overview" title="Student marks and internships" description="Use the Excel-style filters below each column to narrow this cycle’s student progress table." breadcrumb={breadcrumb} lastUpdated={lastUpdated} />
    ...
    {marksLoading || attendanceLoading ? <div className="mt-6 space-y-2">{Array.from({ length: 8 }).map((_, i) => <Skeleton key={i} className="h-10 w-full rounded-none" />)}</div> : !rows.length ? <div className="mt-6"><EmptyState title="No students match these filters" description="Try removing one or more filters to see students." /></div> : <div className="mt-6 overflow-x-auto"><table className="data-grid"><thead><tr><th><p>Student</p><Input aria-label="Filter student" className="mt-2 min-w-[180px] text-xs normal-case" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Name or email" /></th><th><HeaderFilter label="School" value={schoolId} onChange={(value) => { setSchoolId(value); setDepartmentId(''); }} options={schools.map((school) => [school.id, school.name])} /></th><th><HeaderFilter label="Department" value={departmentId} onChange={setDepartmentId} options={visibleDepartments.map((department) => [department.id, department.name])} /></th><th><HeaderFilter label="Internship" value={internship} onChange={setInternship} options={[['research', 'Research'], ['crcs_opportunity', 'CRCS opportunity'], ['self_internship', 'Self-internship'], ['none', 'Not selected']]} /></th><th><HeaderFilter label="Status" value={internshipStatus} onChange={setInternshipStatus} options={STATUS_OPTIONS} /></th><th><HeaderFilter label="Attendance" value={attendanceFilter} onChange={setAttendanceFilter} options={[['recorded', 'Recorded'], ['not_recorded', 'Not recorded'], ['75_plus', '75% and above'], ['50_to_74', '50–74%'], ['below_50', 'Below 50%']]} /></th>{requirementColumns.map((requirement) => <th key={requirement.id} className="text-right"><HeaderFilter label={requirement.title} value={markFilters[requirement.id] ?? ''} onChange={(value) => setMarkFilters((current) => ({ ...current, [requirement.id]: value }))} options={[['entered', 'Score entered'], ['missing', 'No score']]} align="right" /></th>)}<th className="text-right">Total</th></tr></thead><tbody>{rows.map((row) => { const status = statusDetails(row.internship.status); return <tr key={row.student.id}><td><p className="font-semibold text-slate-900">{row.student.full_name}</p><p className="text-xs text-slate-500">{row.student.email}</p></td><td className="text-slate-600">{row.school?.name ?? '—'}</td><td className="text-slate-600">{row.department?.name ?? '—'}</td><td className="font-medium text-slate-800">{row.internship.label}</td><td>{row.internship.status ? <><Badge status={status.tone}>{status.label}</Badge>{status.detail && <p className="mt-1 text-xs text-slate-500">{status.detail}</p>}</> : '—'}</td><td><AttendanceCell attendance={row.attendance} /></td>{requirementColumns.map((requirement) => <td key={requirement.id} className="num text-right font-semibold text-slate-800">{row.assessment?.requirements?.find((item) => item.id === requirement.id)?.score ?? '—'}</td>)}<td className="num text-right font-bold text-brand-700">{row.assessment ? `${row.assessment.total} / ${row.assessment.maximum}` : '—'}</td></tr>; })}</tbody></table></div>}
  </div>;
}
