...
export default function SelfInternshipReviewPage() {
  ...
  const { data: internship, refetch, error } = useQuery({
    queryKey: ['self-internship-review', activeId],
    queryFn: () => api(`/self-internships/${activeId}`),
    enabled: !!activeId,
  });

  const decisionMutation = useMutation({
    mutationFn: (decision) => api(`/self-internships/${activeId}/mentor-decision`, { method: 'PATCH', body: { decision, reason: reason || undefined } }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['self-internship-review', activeId] }),
  });

  return (
    <Card className="p-4 max-w-lg">
      <h2 className="font-medium mb-3">Review Self-Internship</h2>
      {/* ponytail: no mentor-scoped list endpoint yet — look up the self-internship by id. */}
