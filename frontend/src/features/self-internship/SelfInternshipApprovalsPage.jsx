...
function DecisionModal({ internship, onClose, onDecide, pending, error }) {
  ...
  return <Dialog open onClose={onClose} size="lg" title={internship.company_name}>
    <p className="-mt-3 mb-5 text-sm text-slate-600">Review the student's submission and supporting file before recording a decision.</p>
    <div className="grid gap-4 rounded-xl bg-slate-50 p-4 sm:grid-cols-2"><div><p className="text-xs font-bold uppercase tracking-wide text-slate-500">Student</p><p className="mt-1 font-semibold text-ink">{internship.student?.full_name ?? 'Student'}</p><p className="text-sm text-slate-600">{internship.student?.email ?? 'Email not available'}</p></div><div><p className="text-xs font-bold uppercase tracking-wide text-slate-500">Submitted</p><p className="mt-1 text-sm font-medium text-ink">{dateText(internship.created_at)}</p></div><div><p className="text-xs font-bold uppercase tracking-wide text-slate-500">Company website</p>{internship.company_website ? <a className="mt-1 block break-all text-sm font-medium text-brand-700 underline" href={internship.company_website} target="_blank" rel="noreferrer">{internship.company_website}</a> : <p className="mt-1 text-sm text-slate-600">Not provided</p>}</div><div><p className="text-xs font-bold uppercase tracking-wide text-slate-500">Offer letter</p>{offerLetter?.url ? <p className="mt-1 text-sm font-semibold text-brand-700">{offerLetter.file_name}</p> : <p className="mt-1 text-sm text-red-700">File is missing</p>}</div><div><p className="text-xs font-bold uppercase tracking-wide text-slate-500">HR contact name</p><p className="mt-1 text-sm font-medium text-ink">{internship.hr_name || 'Not provided'}</p></div><div><p className="text-xs font-bold uppercase tracking-wide text-slate-500">HR contact</p><p className="mt-1 break-all text-sm font-medium text-ink">{internship.hr_contact || 'Not provided'}</p></div><div className="sm:col-span-2"><p className="text-xs font-bold uppercase tracking-wide text-slate-500">Company address</p><p className="mt-1 text-sm text-slate-700">{internship.company_address || 'Not provided'}</p></div><div className="sm:col-span-2"><p className="text-xs font-bold uppercase tracking-wide text-slate-500">How the offer was received</p><p className="mt-1 text-sm text-slate-700">{internship.offer_source || 'Not provided'}</p></div></div>
    {offerLetter?.url && <iframe title={`Preview: ${offerLetter.file_name}`} src={documentPreviewUrl(offerLetter)} className="mt-4 min-h-[45vh] w-full rounded-xl bg-slate-100" />}
    ...
    <div className="mt-6 flex flex-wrap justify-end gap-2"><Button variant="ghost" onClick={onClose}>Cancel</Button><Button variant="danger" onClick={() => onDecide('reject', reason)} disabled={pending || !reason.trim()}>{pending ? 'Saving…' : 'Reject request'}</Button><Button onClick={() => onDecide('approve', reason)} disabled={pending}>{pending ? 'Saving…' : 'Approve internship'}</Button></div>
