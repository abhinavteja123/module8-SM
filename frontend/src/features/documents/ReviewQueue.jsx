...
import { FileText, FolderOpen, PaperPlaneTilt } from '@phosphor-icons/react';
...
function FileDialog({ document, onClose }) {
  return <Dialog open onClose={onClose} title={document.file_name} size="lg">
    <p className="text-xs font-bold uppercase tracking-widest text-brand-600">{document.approval_document_type ?? document.report_template?.name ?? 'Student report'}</p>
    <p className="mt-1 text-sm text-slate-600">{document.student?.full_name} · {document.internship?.title ?? 'Internship'}</p>
    <div className="mt-5 rounded-xl bg-slate-50 p-4 text-sm"><p><span className="font-semibold">Submitted:</span> {formatDate(document.uploaded_at)}</p><p className="mt-1"><span className="font-semibold">Deadline:</span> {document.deadline ? `${document.deadline.title} · ${formatDate(document.deadline.due_at)}` : 'Not linked to a deadline'}</p></div>
    <iframe title={`Preview: ${document.file_name}`} src={documentPreviewUrl(document)} className="mt-5 min-h-[55vh] w-full rounded-xl bg-slate-100" />
    <p className="mt-5 text-sm text-slate-600">Use the review section below the tracker to ask the student for an update.</p>
  </Dialog>;
}
...
function SubmissionsDialog({ row, onView, onClose }) {
  ...
  return <Dialog open onClose={onClose} title={row.student?.full_name ?? 'Student'} size="lg">
    <p className="text-xs font-bold uppercase tracking-widest text-brand-600">Submitted documents</p>
    <p className="mt-1 text-sm text-slate-600">{row.internship?.title ?? 'Internship'}</p>
    {!grouped.length ? <p className="mt-6 rounded-lg bg-slate-50 p-4 text-sm text-slate-600">No documents submitted for this internship yet.</p> : <div className="mt-5 space-y-4">{grouped.map(([label, documents]) => <div key={label}><p className="text-xs font-bold uppercase tracking-wide text-slate-500">{label}</p><div className="mt-2 space-y-2">{documents.map((document) => <div key={document.id} className="flex items-center justify-between gap-3 rounded-lg border border-slate-200 p-3"><div className="min-w-0"><p className="truncate text-sm font-semibold text-slate-900">{document.file_name}</p><p className="mt-1 text-xs text-slate-500">Submitted {formatDateTime(document.uploaded_at)}</p>{document.review_comment && <p className="mt-1 text-xs text-amber-800">Feedback sent: {document.review_comment}</p>}</div><Button variant="secondary" className="shrink-0 px-3 py-1.5 text-xs" onClick={() => onView(document)}>View file</Button></div>)}</div></div>)}</div>}
  </Dialog>;
}
...
export default function ReviewQueue() {
  ...
  const breadcrumb = [{ label: 'Home', to: '/faculty' }, { label: 'Student supervision' }, { label: 'Submission tracker' }];
  return <div className="max-w-5xl space-y-6"><PageHeader breadcrumb={breadcrumb} eyebrow="Student supervision" title="Submission tracker" description="One card per student internship. Open a card to see every submitted document and enter marks." action={<span className="inline-flex h-11 w-11 items-center justify-center rounded-2xl bg-brand-50 text-brand-700"><FolderOpen size={22} weight="light" /></span>} />
    ...
    {isLoading && <div className="space-y-3">{Array.from({ length: 3 }).map((_, i) => <Skeleton key={i} className="h-24 w-full" />)}</div>}
    {error && <p className="inline-notice border-red-200 bg-red-50 text-red-700">Submissions could not be loaded. {error.message}</p>}
    {!isLoading && !error && !rows.length && <EmptyState icon={FileText} title={docs.length ? 'No submissions match this student' : 'No student submissions yet'} description={docs.length ? 'Choose another student to see their records.' : 'Report uploads and approval documents will appear here automatically.'} />}
    {rows.length > 0 && <div className="space-y-3">{rows.map((row, index) => { const latest = row.documents[0]; return <motion.div key={row.key} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25, delay: index * 0.04, ease: [0.32, 0.72, 0, 1] }}><Card className="p-5"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="font-semibold text-slate-900">{row.student?.full_name ?? 'Student'}</p><p className="text-xs text-slate-500">{row.student?.email}</p><p className="mt-1 text-xs font-medium text-brand-700">{row.internship?.title ?? 'Internship'}</p></div><div className="flex flex-wrap items-center gap-2"><Badge status={row.documents.length ? 'approved' : 'pending'}>{row.documents.length} file{row.documents.length === 1 ? '' : 's'}</Badge><AttendanceBadge studentId={row.student?.id} /><Button variant="secondary" className="px-3 py-1.5 text-xs" onClick={() => setDetailRow(row)}>View submissions</Button><Button variant="ghost" className="px-3 py-1.5 text-xs text-brand-700" onClick={() => setMarkStudent(row)}>Enter marks</Button></div></div>{latest && <p className="mt-3 text-xs text-slate-500">Last submitted {formatDate(latest.uploaded_at)} · {latest.file_name}</p>}</Card></motion.div>; })}</div>}
    <Card className="border-brand-100 p-6"><div><p className="text-xs font-bold uppercase tracking-widest text-brand-600">Review and notify student</p><h2 className="mt-1 text-xl font-bold text-slate-950">Ask for an update</h2><p className="mt-1 text-sm text-slate-600">Choose a submitted file and describe exactly what the student should correct. They will receive a notification and can upload an updated file before marks are awarded.</p></div><div className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.3fr)_auto] lg:items-end"><div><Label htmlFor="review-file">Submitted file</Label><Select id="review-file" className="mt-2" value={reviewDocumentId} onChange={(event) => { setReviewDocumentId(event.target.value); setMessageSent(false); }}><option value="">Choose a submitted file</option>{visibleDocuments.map((document) => <option key={document.id} value={document.id}>{document.student?.full_name} — {document.file_name}</option>)}</Select></div><div><Label htmlFor="review-message">What should the student update?</Label><textarea id="review-message" className="mt-2 min-h-24 w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm text-ink shadow-sm shadow-slate-900/[0.03] outline-none transition placeholder:text-slate-400 focus:border-brand-500 focus:ring-2 focus:ring-brand-100" value={reviewMessage} onChange={(event) => { setReviewMessage(event.target.value); setMessageSent(false); }} placeholder="For example: Please add the work completed this week and re-upload the synopsis." /></div><Button className="lg:mb-0" onClick={() => sendReviewMessage.mutate()} disabled={!reviewDocumentId || !reviewMessage.trim() || sendReviewMessage.isPending}><PaperPlaneTilt size={15} weight="light" />{sendReviewMessage.isPending ? 'Sending…' : 'Send update request'}</Button></div>{messageSent && <p className="mt-4 rounded-lg bg-emerald-50 px-4 py-3 text-sm font-medium text-emerald-800">Update request sent. The student has been notified.</p>}{sendReviewMessage.isError && <p className="mt-4 rounded-lg bg-red-50 px-4 py-3 text-sm text-red-700">The message could not be sent. {sendReviewMessage.error.message}</p>}</Card>
