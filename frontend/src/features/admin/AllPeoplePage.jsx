...
export default function AllPeoplePage() {
  ...
  const cycleId = validCycleId(selectedCycleId) ? selectedCycleId : null;
  ...
  const params = useMemo(() => {
    const next = cycleId ? new URLSearchParams({ cycle_id: cycleId, page: String(page), page_size: String(pageSize) }) : new URLSearchParams();
  ...
  return <div className="max-w-6xl space-y-6"><PageHeader breadcrumb={[{ label: 'CRCS', to: '/crcs' }, { label: 'All People' }]} eyebrow={`Directory${selectedCycle ? ` · ${selectedCycle.name}` : ''}`} title="All People" description="Only people enrolled in the selected cycle are shown. Search and filters are server-bounded for large directories." />
    <Card className="p-5"><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4"><Input aria-label="Search people" value={search} onChange={(event) => updateFilter(setSearch)(event.target.value)} placeholder="Search name, email, or roll number" /><Select aria-label="Filter by role" value={role} onChange={(event) => updateFilter(setRole)(event.target.value)}><option value="">All roles</option><option value="student">Students</option><option value="faculty">Faculty mentors</option><option value="faculty_coordinator">Faculty coordinators</option><option value="hod">HODs</option><option value="dean">Deans</option><option value="school_office">School office</option><option value="crcs_coordinator">CRCS coordinators</option><option value="crcs_superadmin">CRCS superadmins</option></Select><Select aria-label="Filter by school" value={schoolId} onChange={(event) => { updateFilter(setSchoolId)(event.target.value); setDepartmentId(''); }}><option value="">All schools</option>{schools.map((school) => <option key={school.id} value={school.id}>{school.name}</option>)}</Select><Select aria-label="Filter by department" value={departmentId} onChange={(event) => updateFilter(setDepartmentId)(event.target.value)}><option value="">All departments</option>{visibleDepartments.map((department) => <option key={department.id} value={department.id}>{department.name}</option>)}</Select></div><div className="mt-4 flex flex-wrap items-center justify-between gap-3"><p className="text-sm text-slate-600">{isLoading ? 'Loading people…' : pageText(page, pageSize, total)}</p><div className="flex items-center gap-3"><label className="flex items-center gap-2 text-sm text-slate-600">Per page<Select aria-label="People per page" value={String(pageSize)} onChange={(event) => { setPageSize(Number(event.target.value)); setPage(1); }} className="w-20"><option value="25">25</option><option value="50">50</option><option value="100">100</option><option value="200">200</option></Select></label><div className="flex gap-2"><Button variant="secondary" onClick={() => setPage((value) => Math.max(1, value - 1))} disabled={page <= 1 || isLoading}>Previous</Button><Button variant="secondary" onClick={() => setPage((value) => Math.min(lastPage, value + 1))} disabled={page >= lastPage || isLoading}>Next</Button></div></div></div></Card>
    {!cycleId ? <Card className="border-amber-200 bg-amber-50 p-5 text-sm text-amber-900"><p className="font-semibold">No internship cycle is open.</p><p className="mt-1">Open or create a cycle in Cycle Setup before viewing enrolled people.</p></Card> : error ? <Card className="border-red-200 bg-red-50 p-5 text-sm text-red-800"><p className="font-semibold">The paginated people endpoint is not ready.</p><p className="mt-1">{error.message}</p></Card> : <PeopleDirectory users={users} schools={schools} departments={departments} studentRecords={studentRecordsData?.records ?? []} cycleParticipantTypes={cycleParticipantTypes} onChanged={refresh} showHeading={false} activeFilter={Boolean(search.trim() || role)} />}
  </div>;
}
