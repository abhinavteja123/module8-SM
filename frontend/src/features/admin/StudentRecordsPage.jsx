...
function StudentRecordRow({ record, onOpen }) {
  ...
  return <tr className="align-top hover:bg-slate-50/80">
    ...
    {REPORTS.map(([key]) => <td key={key} className="px-3 py-4"><ReportStatus report={record.reports?.[key]} compact /></td>)}
  </tr>;
}
...
export default function StudentRecordsPage() {
  ...
  const { data, isLoading, error } = useQuery({ queryKey: ['admin-student-records', selectedCycleId], queryFn: () => api(`/admin/student-records?cycle_id=${selectedCycleId}`), enabled: !!selectedCycleId });
  ...
  return <div><PageHeader eyebrow={`Student oversight${selectedCycle ? ` · ${selectedCycle.name}` : ''}`} title="Student records" description="A compact, cycle-specific table. Select a student to see their organisation map and every submitted document." />
    ...
    {isLoading ? <p className="mt-6 text-sm text-slate-500">Loading student records…</p> : error ? <p className="mt-6 text-sm text-red-700">{error.message}</p> : !visibleRecords.length ? <div className="mt-6"><EmptyState title="No students match these filters" description="Try removing a filter to see more student records." /></div> : <Card className="mt-6 overflow-hidden"><table className="w-full table-fixed text-sm"><colgroup><col className="w-[18%]" /><col className="w-[11%]" /><col className="w-[15%]" /><col className="w-[13%]" /><col className="w-[10.75%]" /><col className="w-[10.75%]" /><col className="w-[10.75%]" /><col className="w-[10.75%]" /></colgroup><thead className="bg-slate-50 text-left text-[10px] font-bold uppercase tracking-wide text-slate-500"><tr><th className="px-3 py-3">Student</th><th className="px-3 py-3">Roll number</th><th className="px-3 py-3">Internship type</th><th className="px-3 py-3">Approval stage</th>{REPORTS.map(([, label]) => <th key={label} className="px-3 py-3">{label}</th>)}</tr></thead><tbody className="divide-y divide-slate-100">{visibleRecords.map((record) => <StudentRecordRow key={record.student.id} record={record} onOpen={setSelectedRecord} />)}</tbody></table></Card>}{selectedRecord && <StudentOrganisationDialog record={selectedRecord} onClose={() => setSelectedRecord(null)} />}
  </div>;
}
