...
const breadcrumb = [{ label: 'Home', to: '/coordinator' }, { label: 'Student Records' }];
...
function StudentOrganisationDialog({ record, requirements, onClose }) {
  ...
  return <Dialog open onClose={onClose} title={record.student.full_name} size="xl">
      <p className="-mt-3 mb-5 text-sm text-slate-600">{record.profile?.roll_number ?? 'No roll number'} · {record.student.email}</p>
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{people.map(([label, value]) => <div key={label} className="rounded-xl border border-slate-200 bg-slate-50 p-3"><p className="text-xs font-bold uppercase tracking-wide text-slate-500">{label}</p><p className="mt-1 text-sm font-semibold text-slate-900">{value ?? 'Not assigned yet'}</p></div>)}</div>
      ...
      {record.internship?.path === 'self_internship' && <section className="mt-5 rounded-xl border border-slate-200 bg-slate-50 p-4"><p className="text-xs font-bold uppercase tracking-wide text-slate-500">Employer HR contact</p><div className="mt-3 grid gap-3 sm:grid-cols-2"><div><p className="text-xs font-semibold text-slate-500">Name</p><p className="mt-1 text-sm font-semibold text-slate-900">{record.internship.hr_name || 'Not provided'}</p></div><div><p className="text-xs font-semibold text-slate-500">Contact</p><p className="mt-1 break-all text-sm font-semibold text-slate-900">{record.internship.hr_contact || 'Not provided'}</p></div></div></section>}
      {record.internship?.outcome && <section className="mt-5 rounded-xl border border-slate-200 bg-slate-50 p-4"><p className="text-xs font-bold uppercase tracking-wide text-slate-500">Internship outcome details</p><div className="mt-3 grid gap-3 sm:grid-cols-3">
        <div><p className="text-xs font-semibold text-slate-500">Paid / Unpaid</p><p className="mt-1"><Badge status={record.internship.outcome.nature === 'paid' ? 'approved' : 'pending'}>{record.internship.outcome.nature === 'paid' ? 'Paid' : 'Unpaid'}</Badge></p></div>
        <div><p className="text-xs font-semibold text-slate-500">Stipend</p><p className="mt-1 text-sm font-semibold text-slate-900">{record.internship.outcome.nature === 'paid' && record.internship.outcome.stipend_amount != null ? `₹${record.internship.outcome.stipend_amount}` : '—'}</p></div>
        <div><p className="text-xs font-semibold text-slate-500">Mode</p><p className="mt-1 text-sm font-semibold capitalize text-slate-900">{record.internship.outcome.mode ?? '—'}</p></div>
        <div><p className="text-xs font-semibold text-slate-500">Duration</p><p className="mt-1 text-sm font-semibold text-slate-900">{record.internship.outcome.duration_months != null ? `${record.internship.outcome.duration_months} months` : '—'}</p></div>
        <div><p className="text-xs font-semibold text-slate-500">Company country</p><p className="mt-1 text-sm font-semibold text-slate-900">{record.internship.outcome.company_country ?? '—'}</p></div>
        <div><p className="text-xs font-semibold text-slate-500">Domain / sector</p><p className="mt-1 text-sm font-semibold text-slate-900">{record.internship.outcome.domain_sector ?? '—'}</p></div>
      </div></section>}
...
export default function StudentRecordsPage() {
  ...
  const { data, isLoading, error, dataUpdatedAt } = useQuery({ queryKey: ['admin-student-records', selectedCycleId, page, pageSize, search, track, internshipStatus, documents], queryFn: () => api(`/admin/student-records?${params}`), enabled: !!selectedCycleId, retry: false });
