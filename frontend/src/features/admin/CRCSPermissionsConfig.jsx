...
export default function CRCSPermissionsConfig({ embedded = false }) {
  ...
  return (<div className={embedded ? '' : 'max-w-3xl'}>
    {!embedded && <PageHeader breadcrumb={[{ label: 'CRCS', to: '/crcs' }, { label: 'Access management' }]} eyebrow="Access management" title="Set coordinator access" description="Choose a CRCS Coordinator, then select exactly what they can view and manage. Changes take effect immediately." />}
    {isLoading ? <Card className="space-y-3 p-6"><Skeleton className="h-4 w-40" /><Skeleton className="h-10 w-full" /><Skeleton className="h-24 w-full" /></Card> : !coordinators.length ? <EmptyState title="No CRCS Coordinator found" description="Create a user with the CRCS Coordinator role first, then return here to grant access." /> : <Card className="p-6">
