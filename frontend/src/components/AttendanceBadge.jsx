...
export default function AttendanceBadge({ studentId }) {
  const { data } = useQuery({ queryKey: ['attendance', studentId], queryFn: () => api(`/attendance/${studentId}`), enabled: Boolean(studentId), retry: false });
